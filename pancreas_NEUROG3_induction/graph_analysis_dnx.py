...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  1 10:42:54 2021
...
graph_ts_dict={}
for ft in filt:
    nx_path = f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/networkx/{ft}'
...
for graph_ in graph_list:
    ...
    if  graph_[0]=='static':
        ...
        ft=graph_[2]            
        t=graph_[0]
        nx_path = f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/networkx/{ft}/{t}hpi all'
        print(graph_,'\n',nx_path)
    ...
    if ft in ['all_tfs', 'de_tfs']:
        ...
        for tree_root in root_list:
            if tree_root in graph.nodes():
                ...
                continue
            all_edge = [x for x in list(nx.edge_bfs(graph.subgraph([x for x in tree.nodes]), tree_root))]
...
rna_vsd = pd.read_table(f'inputs/geneExpression/RNAseq_8_DESeq2_VSDcounts.txt',sep='\t', index_col=0)
...
ft='all'
pred_path=f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/networkx/{ft}/GRN_prediction'
...
for tfa in [True, False]:
    ...
    for p in ['all', 'subset', 'hormone_TF']:
        ...
        heat_st.fig.suptitle(f'pancreas heatmap pre_vsed NEUROG3 KO_static {p}')
...
for tfa in [True, False]:
    ...
    for i,g in enumerate(graph_ts_dict[ft]):
        ...
        print(time)
        if i==0:
            sim_ts_df=perturbation_simulation (G_s, exp_df=rna_vsd100_mean, basis_cond=i+1, target_ini=target, exp_type='consistent', perturb_initial='min', threshold=threshold, iteration=iteration)
            target_ini_df=sim_ts_df[sim_ts_df.iloc[:,-1]!=0].iloc[:,-1]
            target_perturb_initial=sim_ts_df[sim_ts_df.index.isin(target)].iloc[:,-1].values
    ...
    for p in ['all', 'subset', 'hormone_TF']:
        ...
        heat_ts.fig.suptitle(f'pancreas heatmap pre_vsed NEUROG3 KO_dynamic {p}')
