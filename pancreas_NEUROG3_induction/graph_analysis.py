...
nx_path = 'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/networkx'
try:
    os.mkdir(nx_path)
except FileExistsError:
    pass

for cond_time in [8,24,48,72]:
...
def network_construct(cond_time):
    ...
    try:
        core_trn = pd.read_table(f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/{cond_time}hpi_Cores/Core_prior_atac_Miraldi_q_ChIP_bias10_maxComb_fdr5_Pancreas_NEUROG3_{cond_time}hpiSet_All_ChIP_sp.tsv',sep='\t')
        comb_cut01 = pd.read_table(f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/prior_atac_Miraldi_q_ChIP_bias10_maxComb_cut01_sp.tsv')
        ...
        core_trn = pd.read_table(f'outputs/networks_targ0p05_SS50_bS5/Network0p05_10tfsPerGene/prior_atac_Miraldi_q_ChIP_bias10_maxComb/{cond_time}hpi_Cores/Core_prior_atac_Miraldi_q_ChIP_bias10_maxComb_fdr5_Pancreas_NEUROG3_{cond_time}hpiSet_All_sp.tsv',sep='\t')
        # construct graph from dataframe
        # analyze graph from networkx
        df_G = core_trn.copy()
    ...
    # dict of gene vsd
    if cond_time==8:
        rna_vsd = pd.read_table(f'inputs/geneExpression/RNAseq_8_DESeq2_VSDcounts.txt',sep='\t', index_col=0, usecols=[0]+list(range(cond_time//8,cond_time//8+6)))
    else:
        rna_vsd = pd.read_table(f'inputs/geneExpression/RNAseq_8_DESeq2_VSDcounts.txt',sep='\t', index_col=0, usecols=[0]+list(range(cond_time//4+1,cond_time//4+6)))
    ...
    rna_de = pd.read_table(f'/Users/user/Desktop/big-data_analysis/NGS_XZ/NGS/XZ_03-04-08-12-2020_RNA-seq_Pancreas/Counts_TPM_mat/analysis/results/WT_pan_{cond_time}hpi.txt',sep='\t', index_col=0).fillna(0)
